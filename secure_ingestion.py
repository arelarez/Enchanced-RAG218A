...
DIR_STAGING = os.environ.get("DIR_STAGING", "./docs_staging")
DIR_INFECTED = os.environ.get("DIR_INFECTED", "./docs_infected")
DIR_PROCESSED = os.environ.get("DIR_PROCESSED", "./docs_processed")
...
def setup_directories():
    """Ensure all DMZ zones are available in the file system."""
...
def generate_file_hash(filepath: str) -> str:
    """Generating digital fingerprints (SHA-256) for document forensics."""
...
def chunk_text(text: str, chunk_size: int = 400, overlap: int = 50) -> list:
    """Breaking text into smaller chunks with contextual overlap."""
    words = text.split()
    chunks = []
...
def secure_ingest_documents(chroma_collection, chunk_size=400, overlap=50):
    """
    Scan, fragment, and insert documents from the Staging zone into Vector DB.
Infected documents or chunks will be quarantined.
    """
    setup_directories()
    ...
    for filepath in files:
        ...
        if safe_chunks:
            ...
            print(f"[SUCCESS] {len(safe_chunks)} safe chunks from {filename} have been ingested.")
