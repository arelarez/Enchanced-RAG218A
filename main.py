import threading
from typing import List, Dictionary, Any, Optional
...
SECRET_API_KEY = "arel_secure_key_2026"
...
self.client = ChromaDB.PersistentClien
t(path=persist_directory)
        self.embedding_fn =
embedding_function.SentenceTransformersEmbeddi
ngFunction(model_name=embedding_model_name)
...
embedding_function=self.embedding_fn)
    ...
    def add_docs(self, ids: List[str],
docs: List[str], metadatas:
...
res['document'][0][i],
                    'metadata':
res['metadatas'][0][i] if res['metadatas']
else {}
                })
        return out
...
def make_rag_prompt(instruction):
str, user_input: str, retrieved:
List[Dict[str,Any]]) -> str:
    context = ""
    for i, r in enumerate(retrieved):
        context += f"[DOC {i+1} — 
id={r.get('id')}]: {r.get('document')} "

    prompt = (
        f"Use the following retrieved
documents to answer the instruction. "
        f"If the document do not contain
relevant information, clearly say you don't
know.\n\n"
        f"{context}\n\n"
        f"Input:\n{user_input}\n\n"
        f"Response:\n"
